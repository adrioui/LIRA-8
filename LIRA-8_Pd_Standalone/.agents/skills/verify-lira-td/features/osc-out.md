...
- `out-meter` streams `/lira/cpu` and `/lira/water-lvl` continuously, even with the instrument idle, and the six audio descriptors stream with them, so eight buses publish without anything being touched.
...
- No other process owns 9121. When TouchDesigner's `lira_osc` holds it, read the `lira_osc` CHOP through the webserver instead.
...
- 9121 has a single owner. When TouchDesigner's OSC In CHOP holds it, `listen` cannot bind and fails by design. Read the CHOP through the webserver instead.
- Eight buses stream on their own: `/lira/cpu`, `/lira/water-lvl`, and the six audio descriptors. The rest are events, so a fresh instance shows nothing else until a control moves.
...
- 43 of the 65 buses are read-only readouts the engine owns, not controls. The six audio buses are readouts too, so 22 of the 65 accept a write.
